...
## Solution
For an example query like the one below, applying a spatial filter over the Juneau ice field in Alaska for a specific time period:
...
## Discussion
The "https" links enable data download with no egress charges to the data user. "S3" object urls can be used for direct S3 access within AWS region us-west-2.
Parsing the urls is also a way to get at the filenames for your granule query results.